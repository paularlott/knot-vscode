...
- **Your spaces only** — lists just the spaces you own in each server's zone (not other users' or shared spaces).
...
3. Enter the server URL, an optional display name, and an API token.
   - Create a token in the Knot web UI (*Profile → Tokens*) or with the CLI: `knot admin` → tokens.
