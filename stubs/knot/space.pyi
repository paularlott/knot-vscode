...
def list(all_zones: bool = ...) -> builtins.list[dict[str, Any]]:
...
def is_running(name: str) -> bool:
    """Check if a space is running. A freshly started space reports running before its agent connects — see is_ready() for the state required by run() and file operations."""
    ...
def is_ready(name: str) -> bool:
    """Check if a space is running with its agent connected. True only when run(), read_file(), and other agent-backed calls will succeed; false while a starting space's agent is still connecting."""
    ...
def wait_for_start(name: str, timeout: int = 30, interval: int = 2) -> bool:
    """Wait for a space to be running with its agent connected (ready). Returns True immediately if already ready (never stops the space); polls every interval seconds until the agent registers or timeout expires. Returns False on timeout."""
    ...
def usage_current(name: str) -> dict[str, Any]:
    """Get the current resource usage point for a space"""
    ...
def usage_history(name: str, range: str = ...) -> builtins.list[dict[str, Any]]:
...
def get_dependencies(name: str) -> builtins.list[str]:
    ...
    ...
def set_dependencies(name: str, depends_on: builtins.list[str]) -> bool:
...
def transfer(name: str, user_id: str) -> bool:
    ...
    ...
def share(name: str, user_ids: builtins.list[str]) -> bool:
...
def unshare(name: str, user_id: str | None = ...) -> bool:
    ...
    ...
def run(name: str, command: str, args: builtins.list[str] | None = ..., timeout: int = ..., workdir: str = ...) -> dict[str, Any]:
    """Execute a command in a space"""
    ...
def run_script(name: str, script_name: str, args: builtins.list[str] | None = ...) -> dict[str, Any]:
    """Execute a script in a space"""
    ...
def eval(name: str, code: str, args: builtins.list[str] | None = ...) -> dict[str, Any]:
